...
class ProjectDBMigrationTest {
    ...
    fun migration1to2_containsCorrectData() {
        ...
        db.query("SELECT * FROM projects_table WHERE id = 1").use { cursor ->
            ...
            assertThat(cursor.getString(cursor.getColumnIndexOrThrow("title"))).isEqualTo("First Project")
            assertThat(cursor.getString(cursor.getColumnIndexOrThrow("description"))).isEqualTo("A description for the first project.")
        ...
        db.query("SELECT * FROM days_table WHERE projectId = 1").use { cursor ->
            assertThat(cursor.moveToFirst()).isTrue()
            assertThat(cursor.getString(cursor.getColumnIndexOrThrow("image"))).isEqualTo("/path/to/image.jpg")
            assertThat(cursor.getString(cursor.getColumnIndexOrThrow("comment"))).isEqualTo("A comment for the day.")
